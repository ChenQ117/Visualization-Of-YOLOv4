...
class Evaluator(object):
    def __init__(self, model=None, visiual=None, showatt=False):
    ...
    def APs_voc(self):
        file = '/data0/BigPlatform/YOLOv4-pytorch/YOLOv4-pytorch/data/test.txt'
        # filenames = os.listdir("/data0/BigPlatform/YOLOv4-pytorch/YOLOv4-pytorch/data/imgs")
        filename1=str(self.visiual).split("/")[-1]
    ...
    def Single_APs_voc(self, img_ind):
        img_path = os.path.join(self.visiual)
        # print(img_path+"!!!!!!!!!")
        img = cv2.imread(img_path)
        bboxes_prd = self.get_bbox(img, self.multi_scale_test, self.flip_test,"val",img_path)
    ...
    def get_bbox(self, img, multi_test=False, flip_test=False, mode='val' , img_path=None):
        # print(img_path+"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        if multi_test:
            ...
            for test_input_size in test_input_sizes:
                ...
                bboxes_list.append(
                    self.__predict(img, test_input_size, valid_scale, mode,img_path)
                )
                if flip_test:
                    bboxes_flip = self.__predict(
                        img[:, ::-1], test_input_size, valid_scale, mode,img_path
                    )
                    bboxes_flip[:, [0, 2]] = (
            ...
            bboxes = self.__predict(img, self.val_shape, (0, np.inf), mode,img_path)

        bboxes = nms(bboxes, self.conf_thresh, self.nms_thresh)

        return bboxes

    def __predict(self, img, test_shape, valid_scale, mode,img_path):
        org_img = np.copy(img)
        ...
        if self.showatt and len(img) and mode == 'val':
            print("hot!!!!!")
            self.__show_heatmap(atten, org_img,img_path)
        return bboxes

    def __show_heatmap(self, beta, img,img_path):
        imshowAtt(beta, img,img_path)
    ...
    def __calc_APs(self, iou_thresh=0.5, use_07_metric=False):
        ...
        imagesetfile = '/data0/BigPlatform/YOLOv4-pytorch/YOLOv4-pytorch/data/test.txt'
        APs = {}
        Recalls = {}
        Precisions = {}
